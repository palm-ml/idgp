...
def transform_nni_params(params):
    import math
    r_params = deepcopy(params)
    pow_list = ["alpha", "beta", "gamma", "theta", "delta", "eta"]
    for k in pow_list:
        r_params[k] = math.pow(10, params[k])
    return r_params

def params_to_args(args, params):
    args.lr, args.wd, args.lr_g, args.wd_g, args.warm_up = params['lr'], params['wd'], params['lr_g'], params['wd_g'], params['warm_up']
    args.alpha, args.beta, args.gamma, args.theta = \
        params['alpha'], params['beta'], params['gamma'], params['theta']
    args.T_1, args.T_2 = params['T_1'], params['T_2']
    return args
...
def extract_args():
    ...
    # parser.add_argument('--update_mode', type=int, default=0)
    # parser.add_argument('--warm_up_mode', type=int, default=0)
...
def weighted_crossentropy_g_with_f(g_outputs, f_outputs, targets, eps=1e-12):
    ...
    loss = ( - torch.sum(l)) / (l.size(0) * l.size(1)) + \
        ( - torch.sum(targets * torch.log(g_outputs) + (1 - targets) * torch.log(1 - g_outputs))) / (l.size(0) * l.size(1))
    return loss
